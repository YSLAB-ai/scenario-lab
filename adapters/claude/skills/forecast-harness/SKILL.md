...
6. Draft curated evidence with `forecast-harness draft-evidence-packet`, trim the packet in place with `forecast-harness curate-evidence-draft`, and assemble a grouped approval view with `forecast-harness draft-approval-packet`.
7. Finalize the revision with `forecast-harness approve-revision --assumption ...`, refresh outputs with `forecast-harness simulate`, and prefer `forecast-harness summarize-revision` / `forecast-harness summarize-run` before opening full report files.
