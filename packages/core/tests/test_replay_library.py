...
def test_builtin_replay_library_covers_repo_owned_domains() -> None:
    ...
    assert len({case.run_id for case in cases}) == 18
    assert corpus.case_count == 18
    assert corpus.anchored_case_count == 6
    assert corpus.domain_counts["interstate-crisis"] == 4
    assert corpus.domain_counts["company-action"] == 3
...
def test_builtin_replay_suite_and_calibration_summary_are_structured(tmp_path: Path) -> None:
    ...
    assert summary.historically_anchored_case_count == 6
    assert summary.domain_breakdown["interstate-crisis"]["count"] == 4
    assert summary.domain_breakdown["company-action"]["count"] == 3
...
def test_builtin_replay_cli_commands_emit_structured_payloads(tmp_path: Path) -> None:
    ...
    assert replay_payload.case_count == 18
    ...
    assert summary_payload.case_count == 18
    assert summary_payload.domains_needing_attention == []
    assert summary_payload.historically_anchored_case_count == 6
    ...
    assert corpus_payload["case_count"] == 18
    assert corpus_payload["anchored_case_count"] == 6
    ...
    assert len(case_list_payload) == 18
